...
X Plugin is a host-neutral MCP server and a shared DM workflow skill, with separate Claude Code and Codex manifests. The first version targets local, single-user usage.
...
MCP HTTP authentication uses a distinct user-configured secret. It is not X authorization, OAuth token passthrough, or an implementation of public MCP OAuth. Public hosting must add resource discovery, client authorization, and isolated encrypted token storage before exposing the endpoint.
